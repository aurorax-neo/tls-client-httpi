...
func processRequest(request cycleTLSRequest) (result fullRequest) {
	...
	req, err := fhttp.NewRequest(strings.ToUpper(request.Options.Method), request.Options.URL, strings.NewReader(request.Options.Body))
	if err != nil {
		log.Fatal(err)
	}
	headerorder := []string{}
	...
	req.Header = fhttp.Header{
		...
		fhttp.PHeaderOrderKey: headeOrder,
	}
	//set our Host header
...
func main() {
	...
	var addr *string
	if exists {
		addr = flag.String("addr", ":"+port, "fhttp service address")
	} else {
		addr = flag.String("addr", ":9112", "fhttp service address")
	}
