...
var keyShareCurvesExtensions = map[string]utls.KeyShare{
	"GREASE": {Group: utls.CurveID(utls.GREASE_PLACEHOLDER), Data: []byte{0}},
	"P256":   {Group: utls.CurveP256},
	"P384":   {Group: utls.CurveP384},
	"P521":   {Group: utls.CurveP521},
	"X25519": {Group: utls.X25519},
}
...
func ToTLSExtensions(e *Extensions) (extensions *TLSExtensions) {
	...
	if e.SupportedSignatureAlgorithms != nil {
		...
		for _, s := range e.SupportedSignatureAlgorithms {
			...
			if val, ok := supportedSignatureAlgorithmsExtensions[s]; ok {
				signatureAlgorithms = val
			} else {
				hexInt, _ := strconv.ParseInt(s, 0, 0)
				signatureAlgorithms = utls.SignatureScheme(hexInt)
			}
			extensions.SupportedSignatureAlgorithms.SupportedSignatureAlgorithms = append(extensions.SupportedSignatureAlgorithms.SupportedSignatureAlgorithms, signatureAlgorithms)
		}
	}
	...
	if e.RecordSizeLimit != 0 {
		...
		extensions.RecordSizeLimit = &utls.FakeRecordSizeLimitExtension{Limit: uint16(hexInt)}
	}
	if e.DelegatedCredentials != nil {
		...
		for _, s := range e.DelegatedCredentials {
			...
			extensions.DelegatedCredentials.SupportedSignatureAlgorithms = append(extensions.DelegatedCredentials.SupportedSignatureAlgorithms, signatureAlgorithms)
		}
	}
	...
	if e.SignatureAlgorithmsCert != nil {
		...
		for _, s := range e.SignatureAlgorithmsCert {
			var signatureAlgorithmsCert utls.SignatureScheme
			if val, ok := supportedSignatureAlgorithmsExtensions[s]; ok {
				signatureAlgorithmsCert = val
			} else {
				hexStr := fmt.Sprintf("0x%v", e.RecordSizeLimit)
				hexInt, _ := strconv.ParseInt(hexStr, 0, 0)
				signatureAlgorithmsCert = utls.SignatureScheme(hexInt)
			}
			extensions.SignatureAlgorithmsCert.SupportedSignatureAlgorithms = append(extensions.SignatureAlgorithmsCert.SupportedSignatureAlgorithms, signatureAlgorithmsCert)
		}
	}
