...
// borrowed from https://github.com/caddyserver/forwardproxy/blob/master/httpclient/httpclient.go
import (
	"bufio"
	"context"
	"crypto/tls"
...
func newConnectDialer(proxyURLStr string, UserAgent string) (proxy.ContextDialer, error) {
	...
	client := &connectDialer{
		ProxyURL:          *proxyURL,
		DefaultHeader:     make(fhttp.Header),
...
// DialContext ctx.Value will be inspected for optional ContextKeyHeader{} key, with `fhttp.Header` value,
...
func (c *connectDialer) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	if ctxHeader, ctxHasHeader := ctx.Value(ContextKeyHeader{}).(fhttp.Header); ctxHasHeader {
	...
	connectHTTP1 := func(rawConn net.Conn) (net.Conn, error) {
		...
		resp, err := fhttp.ReadResponse(bufio.NewReader(rawConn), req)
		if err != nil {
			_ = rawConn.Close()
			return nil, err
		}
	...
	case "https":
		if c.DialTLS != nil {
			...
			tlsConf := tls.Config{
				NextProtos:         []string{"h2", "fhttp/1.1"},
