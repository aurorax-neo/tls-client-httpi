...
import (
	...
	"github.com/aurorax-neo/tls_client_httpi/cycletls_fhttp/httptrace"

	"github.com/aurorax-neo/tls_client_httpi/cycletls_fhttp/http2/hpack"
...
const (
	...
	defaultUserAgent = "Go-fhttp-client/2.0"
)
...
type Transport struct {
	...
	// it will be used to set fhttp.Response.TLS.
	...
	// plain-text "fhttp" scheme. Note that this does not enable h2c support.
	AllowHTTP bool
...
// ConfigureTransport configures a net/fhttp HTTP/1 Transport to use HTTP/2.
// It returns an error if t1 has already been HTTP/2-enabled.
//
// Use ConfigureTransports instead to configure the HTTP/2 Transport.
func ConfigureTransport(t1 *fhttp.Transport) error {
	_, err := ConfigureTransports(t1)
	return err
}

// ConfigureTransports configures a net/fhttp HTTP/1 Transport to use HTTP/2.
// It returns a new HTTP/2 Transport for further configuration.
// It returns an error if t1 has already been HTTP/2-enabled.
func ConfigureTransports(t1 *fhttp.Transport) (*Transport, error) {
...
func configureTransports(t1 *fhttp.Transport) (*Transport, error) {
	...
	if !strSliceContains(t1.TLSClientConfig.NextProtos, "fhttp/1.1") {
		t1.TLSClientConfig.NextProtos = append(t1.TLSClientConfig.NextProtos, "fhttp/1.1")
	}
	upgradeFn := func(authority string, c *tls.Conn) fhttp.RoundTripper {
	...
	if m := t1.TLSNextProto; len(m) == 0 {
		t1.TLSNextProto = map[string]func(string, *tls.Conn) fhttp.RoundTripper{
			"h2": upgradeFn,
		}
	} else {
		m["h2"] = upgradeFn
	...
	// the fhttp.Transport's MaxResponseHeaderBytes. They don't
...
type ClientConn struct {
	...
	singleUse  bool                 // whether being used for a single fhttp.Request
...
type clientStream struct {
	...
	trailer    fhttp.Header  // accumulated trailers
	resTrailer *fhttp.Header // client's Response.Trailer
}

// awaitRequestCancel waits for the user to cancel a request or for the done
// channel to be signaled. A non-nil error is returned only if the request was
// canceled.
func awaitRequestCancel(req *fhttp.Request, done <-chan struct{}) error {
	ctx := req.Context()
...
func (cs *clientStream) awaitRequestCancel(req *fhttp.Request) {
...
// needs to be detected by net/fhttp regardless of whether it's its
// bundled version (in h2_bundle.go with a rewritten type name) or
// from a user's x/net/http2. As such, as it has a unique method name
// (IsHTTP2NoCachedConnError) that net/fhttp sniffs for via func
// isNoCachedConnError.
...
func (t *Transport) RoundTripOpt(req *fhttp.Request, opt RoundTripOpt) (*fhttp.Response, error) {
	if !(req.URL.Scheme == "https" || (req.URL.Scheme == "fhttp" && t.AllowHTTP)) {
...
func shouldRetryRequest(req *fhttp.Request, err error, afterBodyWrite bool) (*fhttp.Request, error) {
	if !canRetryError(err) {
		return nil, err
	}
	// If the Body is nil (or fhttp.NoBody), it's safe to reuse
	// this request and its Body.
	if req.Body == nil || req.Body == fhttp.NoBody {
		return req, nil
	}
...
// errRequestCanceled is a copy of net/fhttp's errRequestCanceled because it's not
// exported. At least they'll be DeepEqual for h1-vs-h2 comparisons tests.
var errRequestCanceled = errors.New("net/fhttp: request canceled")

func commaSeparatedTrailers(req *fhttp.Request) (string, error) {
	keys := make([]string, 0, len(req.Trailer))
	for k := range req.Trailer {
		k = fhttp.CanonicalHeaderKey(k)
		switch k {
...
func (cc *ClientConn) responseHeaderTimeout() time.Duration {
	...
	// this for compatibility with the old fhttp.Transport fields when
...
func checkConnHeaders(req *fhttp.Request) error {
...
// means unknown.
func actualContentLength(req *fhttp.Request) int64 {
	if req.Body == nil || req.Body == fhttp.NoBody {
		return 0
	}
...
func (cc *ClientConn) RoundTrip(req *fhttp.Request) (*fhttp.Response, error) {
	resp, _, err := cc.roundTrip(req)
	return resp, err
}

func (cc *ClientConn) roundTrip(req *fhttp.Request) (res *fhttp.Response, gotErrAfterReqBodyWrite bool, err error) {
	...
	handleReadLoopResponse := func(re resAndError) (*fhttp.Response, bool, error) {
		res := re.res
...
// Must hold cc.mu.
func (cc *ClientConn) awaitOpenSlotForRequest(req *fhttp.Request) error {
...
func (cc *ClientConn) requestGzip(req *fhttp.Request) bool {
	// TODO(bradfitz): this is a copy of the logic in net/fhttp. Unify somewhere?
...
func (cc *ClientConn) encodeHeaders(req *fhttp.Request, trailers string, contentLength int64) ([]byte, error) {
	cc.hbuf.Reset()
	...
	enumerateHeaders := func(f func(name, value string)) {
		...
		pHeaderOrder, ok := req.Header[fhttp.PHeaderOrderKey]
		m := req.Method
		if m == "" {
			m = fhttp.MethodGet
		}
		...
		var didUA bool
		var kvs []fhttp.HeaderKeyValues

		if headerOrder, ok := hdrs[fhttp.HeaderOrderKey]; ok {
	...
	enumerateHeaders(func(name, value string) {
		...
		if name == fhttp.PHeaderOrderKey || name == fhttp.HeaderOrderKey {
			return
		}
...
// a "content-length" request header. This logic is basically a copy of the net/fhttp
...
// requires cc.mu be held.
func (cc *ClientConn) encodeTrailers(req *fhttp.Request) ([]byte, error) {
	cc.hbuf.Reset()
...
func (rl *clientConnReadLoop) handleResponse(cs *clientStream, f *MetaHeadersFrame) (*fhttp.Response, error) {
	if f.Truncated {
	...
	header := make(fhttp.Header, len(regularFields))
	res := &fhttp.Response{
		Proto:      "HTTP/2.0",
		ProtoMajor: 2,
		Header:     header,
		StatusCode: statusCode,
		Status:     status + " " + fhttp.StatusText(statusCode),
	}
	for _, hf := range regularFields {
		key := fhttp.CanonicalHeaderKey(hf.Name)
		if key == "Trailer" {
			t := res.Trailer
			if t == nil {
				t = make(fhttp.Header)
			...
			foreachHeaderElement(hf.Value, func(v string) {
				t[fhttp.CanonicalHeaderKey(v)] = nil
			})
		} else {
			vv := header[key]
	...
	if statusCode >= 100 && statusCode <= 199 {
		cs.num1xx++
		const max1xxResponses = 5 // arbitrary bound on number of informational responses, same as net/fhttp
	...
	if !streamEnded || isHead {
		...
		if clens := res.Header["Content-Length"]; len(clens) == 1 {
			if cl, err := strconv.ParseUint(clens[0], 10, 63); err == nil {
				...
				// TODO: care? unlike fhttp/1, it won't mess up our framing, so it's
...
func (b transportResponseBody) Read(p []byte) (n int, err error) {
	...
	if cs.bytesRemain != -1 {
		if int64(n) > cs.bytesRemain {
			...
			if err == nil {
				err = errors.New("net/fhttp: server replied with more than declared Content-Length; truncated")
...
func (rt erringRoundTripper) RoundTrip(*fhttp.Request) (*fhttp.Response, error) { return nil, rt.err }
...
func isConnectionCloseRequest(req *fhttp.Request) bool {
	return req.Close || httpguts.HeaderValuesContainsToken(req.Header["Connection"], "close")
}

// registerHTTPSProtocol calls Transport.RegisterProtocol but
// converting panics into errors.
func registerHTTPSProtocol(t *fhttp.Transport, rt noDialH2RoundTripper) (err error) {
	defer func() {
		if e := recover(); e != nil {
			err = fmt.Errorf("%v", e)
		}
...
// (The field is exported so it can be accessed via reflect from net/fhttp; tested
// by TestNoDialH2RoundTripperType)
type noDialH2RoundTripper struct{ *Transport }

func (rt noDialH2RoundTripper) RoundTrip(req *fhttp.Request) (*fhttp.Response, error) {
	res, err := rt.Transport.RoundTrip(req)
	if isNoCachedConnError(err) {
		return nil, fhttp.ErrSkipAltProtocol
	}
	return res, err
}
...
func traceGetConn(req *fhttp.Request, hostPort string) {
	trace := httptrace.ContextClientTrace(req.Context())
...
func traceGotConn(req *fhttp.Request, cc *ClientConn, reused bool) {
	trace := httptrace.ContextClientTrace(req.Context())
